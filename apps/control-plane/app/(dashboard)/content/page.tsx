import { getAllArticles, getAllPodcasts } from '../../../lib/api';
import { toggleArticleStatus, deleteArticle, togglePodcastStatus, deletePodcast } from '../../actions';
...
export default async function ContentPage(props: {
  ...
  const articlesResult = isArticles || true ? await getAllArticles({ status, page }) : null;
  const podcastsResult = !isArticles ? await getAllPodcasts({ status, page }) : null;
  ...
  const activeCount = isArticles ? (articlesResult?.data?.total ?? 0) : (podcastsResult?.data?.total ?? 0);
  ...
  return (<>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          ...
          <ContentFilter tab={tab} status={status} />
        </div>
